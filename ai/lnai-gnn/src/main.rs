...
use clap::Parser;
use dataset::{
    DEFAULT_KNN_K, DEFAULT_MAX_GROUP, GnnDataset, GnnNormParams, PrefetchBatchedBatcher,
};
...
fn main() -> Result<()> {
    ...
    let (mut model, norm, dataset): (StellarGnn<TrainBackend>, GnnNormParams, GnnDataset) =
        if let Some(resume_dir) = &args.resume_from {
            ...
            let mut loaded_model =
                StellarGnnConfig::new(GNN_INPUT_DIM, args.hidden_dim, GNN_OUTPUT_DIM)
    ...
    for epoch in 1..=args.epochs {
        ...
        while let Some((nodes, adj, targets)) = prefetcher.next_batch::<TrainBackend>(&device) {
            ...
            let loss = compute_gnn_physics_loss(predictions, targets, args.physics_weight);
